...
class Run(csdl.Model):
    ...
    def define(self):
        ...
        ux = self.create_input('ux', val=np.ones((num))*300000)
        uz = self.create_input('uz', val=np.ones((num))*70000)
        ua = self.create_input('ua', val=np.ones((num))*0.1) # pitch angle (theta)

        # initial conditions for states
        self.create_input('vx_0', 1)
        ...
        self.add_design_variable('ux', lower=0, scaler=1E-5)
        self.add_design_variable('uz', lower=0, scaler=1E-5)
...
#print(sim['cruise_thrust'])
#print(sim['lift_thrust'])
#exit()
